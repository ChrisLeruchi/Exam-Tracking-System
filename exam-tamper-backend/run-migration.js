 ...
 * Creates all tables from scratch using correct lowercase table names
...
async function runMigration() {
  ...
  try {
    ...
    // 2. Create users table (if not exists) and add missing columns
    ...
    `
    // Add email and lastLogin columns if they don't exist (for existing tables)
    await prisma.$executeRaw`ALTER TABLE "users" ADD COLUMN IF NOT EXISTS "email" TEXT`
    await prisma.$executeRaw`ALTER TABLE "users" ADD COLUMN IF NOT EXISTS "lastLogin" TIMESTAMP(3)`
